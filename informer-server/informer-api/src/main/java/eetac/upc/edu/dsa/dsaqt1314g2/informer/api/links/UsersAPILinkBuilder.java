...
public class UsersAPILinkBuilder {
	...
	public static final Link buildURIUsers(UriInfo uriInfo, String offset, String length, String rel) {
		URI uriStings;
		if (offset == null && length == null)
			uriStings = uriInfo.getBaseUriBuilder().path(UserResource.class).build(); // devuelve
																						// http:blabla/users
		else {
			uriStings = uriInfo.getBaseUriBuilder().path(UserResource.class).queryParam("o", offset).queryParam("l", length).build();
	...
	public static final Link buildTemplatedURIUsers(UriInfo uriInfo, String rel) {
		URI uriUsers;
		uriUsers = uriInfo.getBaseUriBuilder().path(UserResource.class).queryParam("o", "{offset}").queryParam("l", "{length}").build();
		Link link = new Link();
	...
	public final static Link buildURILibro(UriInfo uriInfo, User user) {
		URI stingURI = uriInfo.getBaseUriBuilder().path(UserResource.class).build();
		Link link = new Link();
	...
	public final static Link buildURIUserName(UriInfo uriInfo, String username, String rel) {
		URI stingURI = uriInfo.getBaseUriBuilder().path(UserResource.class).path(UserResource.class, "getUser").build(username);
		Link link = new Link();
		link.setUri(stingURI.toString());
		link.setRel(rel);
	...
	public final static Link buildURIDeleteUser(UriInfo uriInfo, String username, String rel) {
		URI stingURI = uriInfo.getBaseUriBuilder().path(UserResource.class).path(UserResource.class, "deleteUser").build(username);
		Link link = new Link();
		link.setUri(stingURI.toString());
		link.setRel(rel);
	...
	public final static Link buildURISolicitud(UriInfo uriInfo, String username, String rel) {
		URI stingURI = uriInfo.getBaseUriBuilder().path(UserResource.class).path(UserResource.class, "SolicitudAmigo").build(username);
		Link link = new Link();
		link.setUri(stingURI.toString());
		link.setRel(rel);
	...
	public final static Link buildURIEliminarAmigo(UriInfo uriInfo, String username, String rel) {
		URI stingURI = uriInfo.getBaseUriBuilder().path(UserResource.class).path(UserResource.class, "Deletefriend").build(username);
		Link link = new Link();
		link.setUri(stingURI.toString());
		link.setRel(rel);
	...
	public final static Link buildURIAceptarSolicitud(UriInfo uriInfo, String username, String rel) {
		URI stingURI = uriInfo.getBaseUriBuilder().path(UserResource.class).path(UserResource.class, "AceptarSolicitud").build(username);
		Link link = new Link();
		link.setUri(stingURI.toString());
		link.setRel(rel);
