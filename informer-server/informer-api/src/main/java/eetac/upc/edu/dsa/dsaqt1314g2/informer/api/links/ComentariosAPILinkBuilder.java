...
public class ComentariosAPILinkBuilder {
    public final static Link buildURIRootAPI(UriInfo uriInfo) { //getBase -> http:blablabla/beeter-api/
            ...
            link.setType(MediaType.INFORMER_API_LINK_COLLECTION); //devolver coleccion de enlaces

            return link;
    }

    public static final Link buildURIComentarios(UriInfo uriInfo, String rel) {
            return buildURIComentarios(uriInfo, null, null, null, rel);
    }

    public static final Link buildURIComentarios(UriInfo uriInfo, String offset, String length, String username, String rel) {
            URI uriStings;
            if (offset == null && length == null)
                    uriStings = uriInfo.getBaseUriBuilder().path(ComentarioResource.class).build();        //devuelve http:blabla/stings
            else {
//                    if (username == null)
...
//                                            .queryParam("username", username).build();
    ...
    public static final Link buildTemplatedURIComentarios(UriInfo uriInfo, String rel, boolean username) {
            URI uriStings;
            if (username)
                    uriStings = uriInfo.getBaseUriBuilder().path(ComentarioResource.class).queryParam("offset", "{offset}").queryParam("length", "{length}")
                                    .queryParam("username", "{username}").build();
            else
                    uriStings = uriInfo.getBaseUriBuilder().path(ComentarioResource.class).queryParam("offset", "{offset}").queryParam("length", "{length}").build();
    ...
    public final static Link buildURIComentarioId(UriInfo uriInfo, int libroid, int comentarioid, String rel) {
            URI stingURI = uriInfo.getBaseUriBuilder().path(ComentarioResource.class).path(ComentarioResource.class, "getComentario").build(libroid, comentarioid);
            Link link = new Link();
            link.setUri(stingURI.toString());
            link.setRel(rel);
