		...
		$.each(data.mensajes, function(i, m) {
			if (usuarioAnterior != m.username) {
				if (started != 0)
					...
					started = 1;
				if (started == 1) {
					if (minombre == m.username)
						htmlString += '<div id="mensaje' + m.identificador + '" class="mensaje mio">';
					else
						htmlString += '<div id="mensaje' + m.identificador + '" class=mensaje>';
				}
